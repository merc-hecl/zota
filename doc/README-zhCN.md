...
- 新增同标题笔记导出时的覆盖写入行为。
- 从阅读器视图切换到笔记视图时，侧边栏聊天面板会自动隐藏。
- 修复取消引用后引用内容被意外重新附加的问题。
- 在侧边栏视图和浮窗视图中，消息区域支持通过 `ArrowUp/ArrowDown` 键滚动。
- 精简调试日志输出，减少噪声。
...
- 添加了 **xAI**、**MiniMax** 和 **SiliconFlow** 作为内置供应商。
- 现在可以**重新生成**上一条消息的响应并在不同响应版本之间切换。
  - 在导出聊天历史为**笔记**时，默认导出**最新**版本的响应。
- 现在可以在聊天时**中止** AI 的响应，中止后可以要求 AI **继续**生成响应。
  - 但是 AI 继续生成的响应**不能**保证完美接续之前被中止的响应。**不建议**在需要连续对话的场景中使用。
- 现在可以在**文库界面**通过拖拽单个或多个条目到聊天面板与 AI 模型进行讨论。
  - 在文库界对**多个条目**的聊天历史在导出为**笔记**时，默认导出在文库目录下。
- 现在可以在笔记中正确地显示聊天历史中引用的图片了。
...
- 支持 ~~**多供应商**和~~ **多 API Key**：现在可以配置多个 API 端点和密钥（每个密钥可以设置一个昵称），可以在不同供应商之间切换。
  ![multi-apikey](screenshots/multiapikey_zh.png)
